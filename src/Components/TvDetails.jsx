...
const TvDetails = () => {
  ...
  const releaseYear = info.detail.first_air_date ? info.detail.first_air_date.split("-")[0] : 'N/A';
  ...
  return (
    ...
    >
          ...
          <div className="w-full flex-1 flex flex-col justify-center">
            ...
            <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-3 sm:gap-4 text-zinc-300 font-medium mb-6">
              ...
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-xl backdrop-blur-sm">
                ...
                <span>{info.detail.first_air_date || 'N/A'}</span>
              </div>
              {info.detail.number_of_episodes && (
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-xl backdrop-blur-sm">
                  <i className="ri-list-check text-zinc-400"></i>
      ...
      <div className="w-full max-w-[1600px] mx-auto px-4 ">
        {/* Secondary Info Layout: Watch Providers & Production/Networks */}
        <div className="flex gap-8 mb-4 mt-4 relative">
          {/* Ambient Glow */}
          ...
          {info.watchproviders && (
            <motion.div
              ...
              className="w-fit relative z-10 bg-gradient-to-br from-white/[0.08] to-transparent border border-white/[0.15] backdrop-blur-3xl rounded-[2.5rem] p-8 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-64 h-64 bg-[#6556CD]/20 blur-[80px] rounded-full pointer-events-none"></div>

              <h2 className="text-xl sm:text-2xl  text-white mb-10 flex items-center gap-4 relative z-10">
                <span className="w-12 h-12 rounded-full bg-gradient-to-br from-[#6556CD] to-purple-600 flex items-center justify-center text-white shadow-[0_0_30px_rgba(101,86,205,0.4)]">
                ...
                {info.watchproviders.flatrate && (
                  <div className="flex-1">
                    <h3 className="text-sm font-bold uppercase tracking-[0.25em] text-[#6556CD] mb-6 flex items-center gap-3">
                      <div className="w-2 h-2rounded-full bg-[#6556CD] animate-pulse shadow-[0_0_10px_#6556CD]"></div>
                      Stream
                    </h3>
                    <div className="flex gap-5 flex-wrap">
                      {info.watchproviders.flatrate.map((item, i) => (
                        <div key={i} className="group relative flex flex-col items-center">
                          <div className="w-[50px] h-[50px] rounded-2xl overflow-hidden p-[2px] bg-gradient-to-br from-white/20 to-white/5 hover:from-[#6556CD] hover:to-purple-500 transition-all duration-300 shadow-xl cursor-pointer hover:shadow-[0_0_25px_rgba(101,86,205,0.5)] transform hover:-translate-y-2">
                            <img title={item.provider_name} className="w-full h-full rounded-2xl object-c" src={`https://image.tmdb.org/t/p/w200/${item.logo_path}`} alt={item.provider_name} />
                          </div>
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-8 relative z-10">
                  {info.detail.production_companies.map((company, i) => (
                    company.logo_path ? (
                      <div key={i} className="group relative bg-white/5 backdrop-blur-sm p-2 rounded-2xl border border-white/10 shadow-xl hover:bg-white/10 hover:border-[#6556CD]/50 hover:shadow-[0_0_30px_rgba(101,86,205,0.2)] transition-all duration-500 flex items-center justify-center min-w-[50px] h-[50px] overflow-hidden cursor-pointer transform hover:-translate-y-2">
                        ...
                        <img title={company.name} className="max-h-[40px] max-w-[40px] object-contain relative z-10 drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] group-hover:scale-110 transition-transform duration-500" src={`https://image.tmdb.org/t/p/w200/${company.logo_path}`} alt={company.name} />
                      </div>
                    ) : (
                      <span key={i} className="text-zinc-300 font-bold px-6 py-4 bg-white/5 rounded-2xl border border-white/10 hover:bg-[#6556CD]/10 hover:border-[#6556CD]/50 hover:text-white transition-all cursor-default shadow-xl transform hover:-translate-y-1">
                        {company.name}
                      </span>
                    )
                  ))}
          ...
          <div className="flex overflow-x-auto gap-6 pb-6 scrollbar-hide snap-x px-2">
            {info.detail.seasons && info.detail.seasons.length > 0 ? (
              info.detail.seasons.map((d, i) => {
                if (d.season_number === 0) return null;
                return (
                  <div
                    key={i}
                    className="w-[180px] sm:w-[200px] flex flex-col items-start group shrink-0 snap-start bg-[#1b1a20] rounded-2xl overflow-hidden border border-white/5 hover:border-[#6556CD]/50 shadow-lg hover:shadow-[0_0_20px_rgba(101,86,205,0.2)] transition-all"
                  >
                    <div className="w-full aspect-[2/3] relative bg-zinc-900 overflow-hidden">
                      <img
                        className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                    ...
                    <div className="p-5 w-full">
                      <h3 className="text-white text-lg font-bold leading-tight group-hover:text-[#6556CD] transition-colors line-clamp-1 mb-1">{d.name}</h3>
                      <div className="flex items-center justify-between mt-2">
                        <p className="text-zinc-400 text-sm">{d.episode_count} Episodes</p>
              ...
              <div className="text-zinc-500 font-medium text-lg italic w-full">No Seasons Available</div>
            )}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 sm:gap-8">
            {(info.recommendations.length > 0 ? info.recommendations : info.similar).map((m, i) => (
              <Link
                to={`/${m.media_type || 'tv'}/details/${m.id}`}
                key={i}
