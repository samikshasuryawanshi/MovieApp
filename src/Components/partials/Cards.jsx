...
const Cards = ({data,title}) => {
  ...
  return <div className="grid w-full h-full grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-7 px-5 mt-10">
    {data.map((item,i)=>(
        <Link to={`/${data.media_type || title}/details/${item.id}`} className="group relative w-fit h-fit p-4 bg-[#1b1a20] rounded-2xl transition-all duration-300 hover:scale-105" key={i}>
          <div className="w-full h-[40vh] overflow-hidden rounded-lg relative">
            <img 
              className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" 
              src={
                item.poster_path || item.backdrop_path || item.profile_path || item.logo_path || item.still_path ?
                `https://image.tmdb.org/t/p/w500/${item.poster_path || item.backdrop_path || item.profile_path || item.logo_path || item.still_path}`:noImage
              } 
